...
<div class="container col-12 col-md-8 col-lg-6 col-xl-4">
    ...
    <form action="{{ route('notes.update', $note) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select name="note_type_id" id="note_type_id" class="form-control">
                ...
                @foreach ($noteTypes as $type)
                    <option value="{{ $type->id }}" {{ old('note_type_id', $note->note_type_id) == $type->id ? 'selected' : '' }}>
                        {{ $type->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <select name="priority_id" id="priority_id" class="form-control">
                ...
                @foreach ($priorities as $priority)
                    <option value="{{ $priority->id }}" {{ old('priority_id', $note->priority_id) == $priority->id ? 'selected' : '' }}>
                        {{ $priority->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="d-flex gap-2">
            ...
            <a href="{{ route('notes.edit', $note) }}" class="btn btn-secondary">Limpiar</a>
            <a href="{{ route('notes.show', $note) }}" class="btn btn-secondary">Cancelar</a>
        </div>
    </form>
</div>
@endsection
